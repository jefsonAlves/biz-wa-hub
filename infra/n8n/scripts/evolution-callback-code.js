...
const tenantId = headers['x-tenant-id'] ?? query.tenant_id ?? query.tenantId;
const connectionId = headers['x-connection-id'] ?? query.connection_id ?? query.connectionId;
const token = headers['x-evolution-webhook-token'] ?? query.token ?? '';
const payload = input.body ?? {};
const instanceName = String(payload.instance ?? payload.instanceName ?? headers['x-instance-name'] ?? query.instance ?? query.instanceName ?? '');
...
try {
  ...
  if (eventName === 'messages.upsert') {
    const messagePayload = Array.isArray(data.messages) ? data.messages[0] : data;
    const key = messagePayload.key ?? {};
    const remoteJid = String(key.remoteJid ?? messagePayload.remoteJid ?? data.remoteJid ?? '');
    ...
    const response = await callback('whatsapp.message.received', {
      ...
      name: messagePayload.pushName ?? messagePayload.notifyName ?? data.pushName ?? data.notifyName ?? null,
      ...
      provider_message_id: key.id ?? messagePayload.id ?? data.id ?? null,
      occurred_at: messagePayload.messageTimestamp ? new Date(Number(messagePayload.messageTimestamp) * 1000).toISOString() : new Date().toISOString(),
