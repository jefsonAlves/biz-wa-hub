...
serve(async (req) => {
  ...
  try {
    ...
    if (!secret) return json({ error: "Webhook secret nÃ£o configurado" }, 503);
    ...
    if (typeof payload.event_type !== "string" || !INBOUND_TYPES.has(payload.event_type)) {
      return json({ error: "event_type invÃ¡lido" }, 400);
    }
...
async function handleAccessRequest(
  ...
  if (!connectionId || !conversationId) throw new Error("connection_id e conversation_id sÃ£o obrigatÃ³rios");
  ...
  if (!conversation?.contact_id || conversation.whatsapp_connection_id !== connectionId) {
    throw new Error("conversa ou conexÃ£o incompatÃ­vel");
  }
  ...
  const requestMessage = String(data.request_message ?? "SolicitaÃ§Ã£o de acesso via WhatsApp").slice(0, 500);
